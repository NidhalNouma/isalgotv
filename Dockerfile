...
ARG STRIPE_API_WEBHOOK_SECRET_CONNECT
...
ARG META_API_TOKEN
ARG CTRADER_CLIENT_ID
ARG CTRADER_SECRET_KEY
ARG DERIV_APP_ID
...
ENV STRIPE_API_WEBHOOK_SECRET_CONNECT=${STRIPE_API_WEBHOOK_SECRET_CONNECT}
...
ENV CTRADER_CLIENT_ID=${CTRADER_CLIENT_ID}
ENV CTRADER_SECRET_KEY=${CTRADER_SECRET_KEY}
ENV DERIV_APP_ID=${DERIV_APP_ID}
...
ENV MARKETING_EMAIL_API_KEY=${MARKETING_EMAIL_API_KEY}
...
# Install system dependencies (make for Makefile, gettext for compilemessages)
RUN apt-get update && apt-get install -y --no-install-recommends make gettext \
