...
const SideBar: React.FC<SideBarProps> = ({
  ...
  const [deleteChatFn, setDeleteChatFn] = useState<DeleteChatFn | null>(null);
  ...
  useEffect(() => {
    if (page === "chat")
      if (open && user && !hasFetchedChats.current) {
        retrieveChats();
...
const MinSideBar: React.FC<MinSideBarProps> = ({
  setOpen,
  page,
  changePage,
  openSettings,
    ...
    <Fragment>
      ...
      {user && (
        <Fragment>
          ...
          >
            <Settings className="w-3.5 aspect-auto" />
          </button>
        </Fragment>
      )}
      <a
...
interface FullSideBarProps {
  ...
  page: Page;
  changePage: (newPage: Page) => void;
  setDeleteChatFn: React.Dispatch<React.SetStateAction<DeleteChatFn | null>>;
  openSettings: () => void;
  openTokens: () => void;
}
          ...
          <Fragment>
            <ScrollDiv className="box-content" onBottomReach={retrieveChats}>
              ...
              <Reorder.Group
                ...
                onReorder={(list: ChatSession[]) => setFilteredChats(list)}
                className="px-2"
              >
                {chats &&
                  filteredChats
                    ...
                    .map((chat) => (
                      ...
                      >
                        <HrefChat
                          ...
                          chat={chat}
                          onClose={onClose}
                          setDeleteChatFn={(payload) =>
                            setDeleteChatFn({
                              deleteFn: payload.deleteFn,
...
const Logo: React.FC<{ className?: string }> = ({ className }) => {
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M211.404 5L211.404 102.073" strokeWidth="10" />
      <path
      ...
      <path
        ...
        strokeWidth="15"
      />
      <path d="M90.3241 351.011L90.3241 422.421" strokeWidth="10" />
