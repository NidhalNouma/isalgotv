...
function NewTradeTask({ close }: NewTradeTaskProps) {
  const { step, nextStep, accounts, setSelectedAccount } = NewTaskHook();
  ...
  return (
    <section className="overflow-y-auto h-full max-h-full w-full">
      ...
      <section className="mt-8 bg-transparent">
        {step === 1 ? (
          <Step1 accounts={accounts} setSelectedAccount={setSelectedAccount} />
        ) : (
          <Fragment></Fragment>
        )}
      </section>
...
function Step1({ accounts, setSelectedAccount }: Step1Props) {
  return (
