...
function SideBar({ onClose, open, setOpen, page, changePage }) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.25, ease: open ? "easeIn" : "easeOut" }}
      onAnimationComplete={() => {
        if (!open) setRenderOpen(false);
      }}
      className="overflow-hidden space-y-4 py-4 backdrop-blur-[320px] bg-background/80 rounded-xl h-full flex flex-col relative"
    >
      {!renderOpen ? (
        <Fragment>
          <button
            onClick={() => setOpen(true)}
            className="btn-icon transition-colors ml-auto mr-auto"
          >
            <PanelRight className="w-5 h-5" />
          </button>
          <div className="flex pt-1.5 flex-col w-full justify-start items-start gap-4">
            <button
              className={`px-2 py-0.5 gap-1 btn-icon ml-auto mr-auto rounded-md transition-colors ${
                page === "chat" &&
